...
- CI runs on pushes to `master`, pull requests targeting `master`, and manual dispatch. It executes focused, high-signal checks on `ubuntu-latest`: quality and server API testing (linting, formatting, Rust tests, server smoke tests, client build, and frontend audit), browser E2E with Chromium (application behavior and maintenance tests), and production container image verification. The production-image job builds the multi-stage Docker image, starts the container, authenticates, and verifies its configured provider catalog. The immutable GitHub Pages maintenance shell deploys only when maintenance assets or its workflow change, or through manual dispatch.
...
- Hosted mode starts from certified provider defaults. Use `ANY_WATCH_PROVIDER_OVERRIDES` for focused switches, or set `ANY_WATCH_CONFIG_PATH=/data/config.toml` after placing a reviewed TOML file in the mounted data directory. The server does not read a workstation user config implicitly.
...
- The `any-watch-failover` Worker Route remains `ani.dangphuc.me/*`. Navigation, assets, and `/api/health` receive four seconds for origin response headers; authenticated API routes receive 65 seconds, and `/api/providers/health` receives 70 seconds. Streaming bodies continue after headers arrive. Typed JSON API failures stay scoped to their endpoint, while connection failures and non-JSON proxy 5xx responses select maintenance. The independent shell is served from `https://silent9669.github.io/any-watch/`; if it also fails, navigation receives a controlled plain-text `503` with maintenance and retry headers.
...
- Authenticated `GET` and `POST /api/providers/health` refreshes share a five-minute aggregate cache and coalescing/version protection. At most sixteen provider checks run concurrently, and each provider check is bounded to 60 seconds. If the initial UI health request fails while entries are still `unknown`, they become retryable `unavailable` entries instead of remaining indefinitely in `Checking`.
