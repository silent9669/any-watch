...
- Production currently enables AniZone and AniDB for English and OPhim, KKPhim, and Niniyo for Vietnamese. AniDB fetches through the system `curl` binary with a neutral `ani-desk/1.0` user agent — never a browser-claiming UA or reqwest TLS stack — because `anidb.app` challenges browser-claiming user agents issued by non-browser TLS stacks. A provider is selectable only while its application health check is healthy.
